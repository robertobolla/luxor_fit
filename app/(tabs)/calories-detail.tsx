...
export default function CaloriesDetailScreen() {
  ...
  const loadDayData = async () => {
    ...
    const dayCal = Math.round(healthData.calories || 0);
    setCaloriesData([{ label: t('common.today'), value: dayCal }]);
    setTotalCalories(dayCal);
    setAverageCalories(dayCal);
  };
  ...
  const loadWeekData = async () => {
    ...
    if (result.success && result.data) {
      result.data.forEach((row: any) => { calMap[row.date] = row.calories || 0; });
    }
    ...
    if (todayStr >= dateToStr(start) && todayStr <= dateToStr(end)) {
      ...
      calMap[todayStr] = Math.round(live.calories || 0);
    }
    ...
    for (let i = 0; i < 7; i++) {
      ...
      data.push({ label: weekDays[i] || `D${i + 1}`, value: calMap[dateToStr(d)] || 0, date: d });
    }
    ...
    setAverageCalories(Math.round(total / daysWithData));
  };
  ...
  const loadMonthData = async () => {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      data.push({ label: (i === 1 || i % 5 === 0) ? `${i}` : '', value: calMap[dateToStr(d)] || 0, date: d });
    }
  ...
  const getUnitLabel = () => `${t('common.of')} ${goalCalories.toLocaleString(locale)} kcal`;

  const getStatusText = () => {
    if (viewMode === 'day') {
      const remaining = goalCalories - totalCalories;
      if (remaining > 0) {
        return t('nutrition.dailyGoalRemaining', { calories: remaining.toLocaleString(locale) });
      }
      return t('nutrition.dailyGoalAchieved');
    }
    return t('nutrition.totalCaloriesBurned', { calories: totalCalories.toLocaleString(locale) });
  };

  // ── Chart ────────────────────────────────────────────────────────────────

  const renderChart = () => {
    const maxValue = Math.max(...caloriesData.map(d => d.value), goalCalories) || 1;
  ...
  const renderPeriodSummary = () => {
    ...
    const nonZero = caloriesData.filter(d => d.value > 0);
    ...
    if (viewMode === 'week') {
      return (
        <View style={styles.summarySection}>
          ...
          {caloriesData.filter(d => d.value > 0).reverse().slice(0, 5).map((item, i) => (
            <View key={i} style={styles.summaryRow}>
              <Text style={styles.summaryLabel}>
                {item.date ? item.date.toLocaleDateString(locale, { weekday: 'long', day: 'numeric', month: 'short' }) : item.label}
              </Text>
              <Text style={styles.summaryValue}>{Math.round(item.value).toLocaleString(locale)} kcal</Text>
            </View>
          ))}
        </View>
      );
    ...
    if (viewMode === 'month') {
      return (
        <View style={styles.summarySection}>
          ...
          {caloriesData.filter(d => d.value > 0).reverse().slice(0, 7).map((item, i) => (
            <View key={i} style={styles.summaryRow}>
              <Text style={styles.summaryLabel}>
    ...
    if (viewMode === 'year') {
      return (
        <View style={styles.summarySection}>
          ...
          {caloriesData.filter(d => d.value > 0).map((month, i) => (
            <View key={i} style={styles.summaryRow}>
              <Text style={styles.summaryLabel}>{month.label}</Text>
              <Text style={styles.summaryValue}>{Math.round(month.value).toLocaleString(locale)} kcal</Text>
            </View>
          ))}
        </View>
      );
    ...
    <>
      ...
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>{t('nutrition.burnedEnergy')}</Text>
          <TouchableOpacity>
              ...
              <View style={styles.statsRow}>
                <View>
                  <Text style={styles.statsNumber}>
                    {(viewMode === 'day' ? totalCalories : averageCalories).toLocaleString(locale)}
                  </Text>
...
const styles = StyleSheet.create({
  ...
  progressCircle: { width: 100, height: 100, borderRadius: 50, backgroundColor: '#3d2f1f', justifyContent: 'center', alignItems: 'center' },
