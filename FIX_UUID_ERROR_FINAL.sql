-- SOLUCIÓN FINAL FORZOSA (FIX_UUID_ERROR_FINAL.sql)
-- Este script elimina dependencias con CASCADE y cambia los tipos de columna.
-- NO importa si da warnings, lo importante es que termine sin errores ROJOS.

BEGIN;

-- 1. Eliminar la vista user_stats y CUALQUIER COSA que dependa de ella (CASCADE)
DROP VIEW IF EXISTS public.user_stats CASCADE;

-- 2. Eliminar funciones que podrían estar "bloqueando" (aunque CASCADE debería encargarse de algunas)
...
-- 3. CAMBIO DE TIPO DE COLUMNA (El paso CRÍTICO)
-- Forzamos el cambio a TEXT. Si hay datos, los convertimos.
...
-- 4. Reconstruir índices para asegurar rendimiento
...
-- 5. Restaurar VISTA user_stats (Versión corregida TEXT)
CREATE OR REPLACE VIEW public.user_stats AS
WITH all_users AS (
  SELECT 
    user_id, -- Ya es text en user_profiles
    created_at,
    age,
    fitness_level
  ...
  UNION
  SELECT 
    user_id, -- Ya es text en admin_roles
    created_at,
    NULL::integer as age,
...
-- 6. Restaurar Funciones (Versiones TEXT)
